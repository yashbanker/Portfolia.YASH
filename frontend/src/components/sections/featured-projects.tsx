          ...
          {items.map((p, i) => (
            <motion.article key={p._id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="group relative flex flex-col h-full rounded-2xl overflow-hidden border border-border bg-card hover:border-blue-500/40 transition-all">
                ...
                <div className="p-6 flex-1 flex flex-col">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4 mt-auto">{p.technologies.slice(0, 4).map((t: string) => (<span key={t} className="text-xs px-2 py-1 rounded-md bg-secondary text-secondary-foreground">{t}</span>))}</div>
                  <div className="flex items-center justify-between mt-4">
                    <Link href={`/projects/${p.slug}`} className="text-sm font-medium text-blue-400 inline-flex items-center gap-1 group-hover:gap-2 transition-all">
